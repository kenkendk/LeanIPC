...
When communicating with another process, it is often useful to have different message types being sent, where some also require a response. The IPC layer in LeanIPC offers this functionality, allowing you to send a message (with arguments passed through the type serializer), and read back the response.

The IPC layer allows you to define custom messages and handlers, and is fully thread safe, allowing multiple threads on both sides to communicate without interference. All communication is implemented with `async`, allowing it to scale to a large number of concurrent communications.
...
It is also possible to _send_ instances of a specific class by reference, but it is not possible to get an automatically created proxy for the _receive_ side. You can use the method `RegisterCustomProxyClass` to register your own type that receives the handle for a remote instance:
```csharp
var peer = new RPCPeer(stream);
peer.RegisterCustomProxyClass<Stream>((RPCPeer peer, Type remoteType, long handle) => new MyStreamProxy(peer, remoteType, handle));
```

Also note that arguments that are serialized, are not serialized back. This is particularly important for methods that update arrays, such as the `Stream` method `int Read(byte[] buffer, int offset, int count)`. When calling this method on a remote proxy, the array is serialized, and the count is returned. But the array is not serialized back, leaving it empty on the calling side. One way to fix it is with a static helper metod similar to this:
```csharp
// Declare method to be invoked on the remote side
public static class Helper {
	public static byte[] RemoteRead(Stream source, int maxlength) {
...
// Invoke it from the local side:
var peer = new RPCPeer(stream);
var result = peer.CallRemoteStaticMethod<byte[]>(typeof(Helper).GetMethod(nameof(RemoteRead)), source, 2048);
```

If you invoke such a method remotely, it can pass the array by reference inside the method, and returns a serialized array.

## Built-in type support
LeanIPC has a number of commonly used types supported in the `SerializationHelper` class, such as `IPEndPoint` (decomposed). Please submit a pull request if you have another integrated type that you want to add handling support for.
...
There is also a `RegisterByRefType<T>()` method, which will specify that a type is always passed by reference. However, if you register this, it will not enable the remote side to invoke any methods on it. If you want to allow the remote side to invoke methods, you should use `RegisterLocallyServedType<T>()`. If you want to limit what methods the remote side can invoke, you can pass in a filter method that can reject or allow calls:
```csharp
